...
enum SocialProvider: string implements Enumerator
{
    ...
    {
        return match ($this) {
            self::GOOGLE, self::LINKEDIN, self::PAYPAL, self::TWITTER => true,
        };
    }
     ...
     * real boolean or as the string "true"/"false". X instead returns the confirmed
     * address itself as `confirmed_email` and omits it entirely when the address is
     * unconfirmed, so its presence as a valid address is the assertion.
     *
    ...
    {
        return match ($this) {
            self::GOOGLE, self::LINKEDIN, self::PAYPAL => filter_var(
